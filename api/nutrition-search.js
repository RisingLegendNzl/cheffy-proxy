...
// [MODIFIED] Pipeline: CANONICAL (v1) -> Avocavo -> OFF -> USDA
...
// --- [NEW] Import shared normalizer ---
const { normalizeKey } = require('../scripts/normalize.js');
// --- [NEW] Import generated canonical database ---
// This file is generated by `scripts/build-canon.js` during the prebuild step
...
// [MODIFIED] Cache prefix now includes the canonical version to bust cache on DB updates
...
// ---------- Utilities ----------
// --- [DELETED] Local normalizeKey function is replaced by shared import ---
...
// ---------- [DELETED] Old Canonical DB ----------
// The old, small CANON object has been removed.
// The old `canonical(query)` function has been removed.
...
async function fetchNutritionData(barcode, query, log = console.log) {
  ...
  // [MODIFIED] Use CANON_VERSION in cache key and shared normalizer
  const finalKey = `${CACHE_PREFIX}:final:${normalizeKey(barcode || query || '')}`;
  const cached = await cacheGet(finalKey);
  if (cached) return cached;

  const safeLog = (msg, level = 'DEBUG', tag = 'NUTRI') => {
  ...
  if (query) {
    ...
    const canonHit = canonGet(key);
    if (canonHit) {
      safeLog(`Canonical HIT for: "${query}" (key: ${key})`, 'INFO');
      // Transform from CanonRow schema to the standard response schema
      const out = {
        status: 'found',
        source: 'CANON',
        confidence: 'high',
        version: CANON_VERSION,
        servingUnit: '100g', // All canon data is per 100g
        usda_link: null,
        calories: canonHit.kcal_per_100g,
        protein: canonHit.protein_g_per_100g,
        fat: canonHit.fat_g_per_100g,
        carbs: canonHit.carb_g_per_100g,
        fiber: canonHit.fiber_g_per_100g || 0,
        // Add other fields if they exist, default to null
        saturatedFat: canonHit.fat_saturated_g_per_100g || null,
        sugars: canonHit.sugar_g_per_100g || null,
        sodium: canonHit.sodium_mg_per_100g ? canonHit.sodium_mg_per_100g / 1000 : null // (mg -> g)
      };

      // We trust our own DB, no need for `accept(out)`
      await cacheSet(finalKey, out, TTL_FINAL_MS); // Cache the hit
      return out;
    }
    safeLog(`Canonical MISS for: "${query}" (key: ${key})`);
  }
  // --- END NEW STEP ---


  // --- STEP 2: Fallback to External APIs (Original Logic) ---
  const tasks = [];
  ...
  // [DELETED] Old canonical fallback is no longer needed
