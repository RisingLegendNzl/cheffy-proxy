...
import { CheckCircle, Clock, ChefHat, Flame, Eye } from ‘lucide-react’;
import { COLORS, SPACING, SHADOWS } from ‘../constants’;
import { isInViewport, prefersReducedMotion, animateValue } from ‘../utils/animationHelpers’;

/**

- Living Recipe Cards - Meal card with premium animations
- Features:
- - Breathing animation (slow scale pulse)
- - Hover lift with shadow expansion
- - Macro rings animate from 0 → value on scroll-into-view
- - Flip animation to reveal recipe
- - Green shimmer wash on mark-as-eaten
- - Protein hero badge with pulse glow
    */
    const MealCard = ({
    meal,
    isEaten = false,
    ...
    const [animatedMacros, setAnimatedMacros] = useState({ protein: 0, fat: 0, carbs: 0, calories: 0 });
...
// Animate macros on scroll into view
useEffect(() => {
if (prefersReducedMotion() || hasAnimated) return;

```
const checkVisibility = () => {
  if (cardRef.current && isInViewport(cardRef.current, 0.3)) {
    setHasAnimated(true);

    // Animate each macro value
    ['protein', 'fat', 'carbs', 'calories'].forEach((macro) => {
      animateValue({
        from: 0,
        to: macros[macro],
        duration: 800,
...
// Show shimmer when marked as eaten
useEffect(() => {
if (isEaten) {
setShowShimmer(true);
...
const displayMacros = hasAnimated ? animatedMacros : macros;

return (
<div
ref={cardRef}
className={`rounded-xl overflow-hidden transition-all duration-300 cursor-pointer group ${ isEaten ? 'opacity-70' : 'hover-lift' } ${!prefersReducedMotion() && !isEaten ? 'animate-breathe' : ''}`}
style={{
...
```
    ...
    {/* Shimmer wash effect */}
    {showShimmer && (
      <div className="absolute inset-0 animate-shimmerWash pointer-events-none" />
    )}
  </div>
  ...
  <div className="p-5">
    ...
    <div className="flex items-center space-x-4 mb-4 text-sm">
      <div className="flex items-center" style={{ color: COLORS.error.main }}>
        ...
        <span className="font-bold">{displayMacros.calories}</span>
      </div>
